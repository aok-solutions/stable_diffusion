  ...
  class ExtrasBatchImagesRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `StableDiffusion::ExtrasBatchImagesRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `StableDiffusion::ExtrasBatchImagesRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'image_list')
        if (value = attributes[:'image_list']).is_a?(Array)
          self.image_list = value
        end
      end
    end
    ...
    def list_invalid_properties
      ...
      if @image_list.nil?
        invalid_properties.push('invalid value for "image_list", image_list cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @image_list.nil?
      true
    end
    ...
    def hash
      [resize_mode, show_extras_results, gfpgan_visibility, codeformer_visibility, codeformer_weight, upscaling_resize, upscaling_resize_w, upscaling_resize_h, upscaling_crop, upscaler_1, upscaler_2, extras_upscaler_2_visibility, upscale_first, image_list].hash
    end
